...
import (
	...
	"github.com/admissioncotroller/pkg/mutation"
...
func (g *GardenerSeedAdmissionController) validateExtensionCRDDeletion(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	t
	if err := mutation.ValidateExtensionDeletion(ctx, g.client, logger, receivedReview.Request); err != nil {
		logger.Errorf(err.Error())
		respond(w, nil, admission.Errored(http.StatusBadRequest, err))
		return
	}
...
func (g *GardenerSeedAdmissionController) mutatePods(w http.ResponseWriter, r *http.Request) {
	...
	patches, err := mutation.MutatePod(ctx, g.client, logger, receivedReview.Request)
	if err != nil {
		logger.Errorf(err.Error())
		respond(w, nil, admission.Errored(http.StatusBadRequest, err))
	}
